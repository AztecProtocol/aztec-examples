 ...
 * into the typed arrays required by the PredictionMarketZkTLS contract's
 * `resolve_market` function.
 *
 * Adapted from the zktls-airdrop example for CoinGecko price attestations.
 * The attestation proves a BTC/USD price was fetched from CoinGecko's API.
 *
 * Both response resolves extract the same price value ($.bitcoin.usd),
 * duplicated to satisfy att_verifier_lib's requirement of 2 items.
 */
...
export function parseAttestation(
  ...
  if (priceValue === undefined || priceConfirmValue === undefined) {
    throw new Error(
      "Attestation data must contain 'price' and 'price_confirm' keys",
    );
  }
